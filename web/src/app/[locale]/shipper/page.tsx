...
import { Truck, MapPin, CheckCircle, Navigation, Phone, ShieldAlert, ShoppingBag, Loader2, Sparkles, UserCheck, Power, BarChart3, Compass } from 'lucide-react';
...
export default function ShipperDashboardPage() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      // Calculate stats based on active/completed
      // For mock purposes, let's load sample completed counts
      const savedCount = localStorage.getItem(`shipper_completed_${user?.id}`) || '0';
      const completedCount = parseInt(savedCount, 10);
      setShipperStats({
        completedToday: completedCount,
        shippingFeesEarned: completedCount * 15000,
        codCollected: completedCount * 125000, // mock cod average
      });
    } catch (err) {
  ...
  const claimOrder = async (orderId: string) => {
    if (!isOnline) {
      alert('Vui lòng bật trạng thái Online để nhận đơn hàng');
      return;
    }
  ...
  const completeOrder = async (orderId: string, status: string) => {
    ...
    try {
      ...
      if (res.ok) {
        if (status === 'DELIVERED') {
          // Increment completed counts in localStorage
          const savedCount = localStorage.getItem(`shipper_completed_${user?.id}`) || '0';
          localStorage.setItem(`shipper_completed_${user?.id}`, (parseInt(savedCount, 10) + 1).toString());
        }
        await fetchOrders();
      } else {
        const errData = await res.json();
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between border-b border-zinc-150 pb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-zinc-500 mt-1">
                Tài xế: <span className="font-bold text-zinc-800">{user?.fullName}</span> (SĐT: {user?.phone})
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className={`flex h-10 px-4 items-center gap-1.5 rounded-2xl border font-bold text-xs transition cursor-pointer shadow-sm
              ${
                isOnline
                  ? 'bg-green-55 hover:bg-green-600 border-green-200 text-white shadow-green-500/10'
                  : 'bg-red-500 hover:bg-red-650 border-red-400 text-white shadow-red-500/10'
              }
            `}
          >
            <Power className="h-4 w-4" />
            <span>Trạng thái: {isOnline ? 'Online nhận đơn' : 'Offline nghỉ'}</span>
          </button>
        </div>

        {/* SHIPPER INCOME METRICS STATS */}
        <div className="mt-6 grid grid-cols-3 gap-3.5">
          <div className="rounded-2xl border border-white/40 bg-white/70 p-4 shadow shadow-zinc-200/50 text-center">
            <span className="block text-[10px] text-zinc-400 font-bold uppercase tracking-wider">ĐƠN ĐÃ GIAO</span>
            <p className="text-xl font-black text-zinc-900 mt-1">{shipperStats.completedToday} đơn</p>
          </div>
          <div className="rounded-2xl border border-white/40 bg-white/70 p-4 shadow shadow-zinc-200/50 text-center">
            <span className="block text-[10px] text-zinc-400 font-bold uppercase tracking-wider">SHIP FEE KIẾM ĐƯỢC</span>
            <p className="text-xl font-black text-orange-650 mt-1">{formatPrice(shipperStats.shippingFeesEarned)}</p>
          </div>
          <div className="rounded-2xl border border-white/40 bg-white/70 p-4 shadow shadow-zinc-200/50 text-center">
            <span className="block text-[10px] text-zinc-400 font-bold uppercase tracking-wider">TIỀN MẶT THU COD</span>
            <p className="text-xl font-black text-zinc-700 mt-1">{formatPrice(shipperStats.codCollected)}</p>
          </div>
        </div>
                  ...
                  >
                    <div className="flex justify-between items-start border-b border-zinc-100 pb-3">
                      <div>
                        ...
                        <p className="font-bold text-zinc-900 text-sm mt-0.5">Khách hàng: {order.user.fullName}</p>
                      </div>
                    ...
                    <div className="mt-4 grid md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2 text-sm text-zinc-650">
                        <p className="flex items-start gap-2">
                          ...
                          <span className="font-semibold text-zinc-800">{address?.fullAddress || (typeof order.deliveryAddress === 'object' ? order.deliveryAddress?.fullAddress : order.deliveryAddress)}</span>
                        </p>
                        <p className="flex items-center gap-2">
                          <Phone className="h-4.5 w-4.5 text-zinc-400 shrink-0" />
                          <a href={`tel:${order.user.phone}`} className="text-orange-500 font-bold hover:underline">
                            {order.user.phone}
                          </a>
                        </p>
                        {order.note && (
                        ...
                        <div className="border-t border-zinc-100 pt-2 text-xs">
                          <p className="font-bold text-zinc-400">CHI TIẾT MÓN ĐẶT:</p>
                          <div className="mt-1 space-y-1 text-zinc-500 font-medium">
                      ...
                      {/* SHIPPER REAL MAP NAVIGATION */}
                      <div className="rounded-2xl border border-zinc-150 overflow-hidden bg-zinc-50 h-56 relative shadow-inner">
                        <iframe
        ...
        <div className="mt-8 pt-8 border-t border-zinc-150">
          <h2 className="text-xl font-extrabold text-zinc-900 flex items-center gap-2">
            ...
            Đơn hàng sẵn sàng nhận ({isOnline ? availableOrders.length : 0})
          </h2>

          {!isOnline ? (
            <div className="mt-4 rounded-3xl border border-dashed border-red-200 bg-red-50/20 p-8 text-center">
              <ShieldAlert className="mx-auto h-10 w-10 text-red-500 animate-bounce" />
              <p className="font-bold text-red-700 text-sm mt-3">Tài xế đang Offline nghỉ ngơi</p>
              <p className="text-xs text-red-500 mt-1">Hãy gạt công tắc "Online" ở trên đầu trang để có thể xem và nhận các đơn hàng mới đang chờ giao!</p>
            </div>
          ) : (
            <div className="mt-4 space-y-4">
                    ...
                    >
                      ...
                      <div className="mt-3 text-xs text-zinc-500 space-y-1.5">
                        <p className="flex items-start gap-1">
                          ...
                          <span>{address?.fullAddress || (typeof order.deliveryAddress === 'object' ? order.deliveryAddress?.fullAddress : order.deliveryAddress)}</span>
                        </p>
                      </div>

                      <div className="mt-4 flex items-center justify-between">
                        <span className="text-[10px] text-zinc-400">
                          {order.items.length} món · Phí ship {formatPrice(order.shippingFee)}
                        </span>
                        <Button
                          size="sm"
